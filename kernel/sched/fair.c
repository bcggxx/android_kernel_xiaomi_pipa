					...
					struct find_best_target_env *fbt_env)
{
	...
	prefer_high_cap = fbt_env->boosted || schedtune_prefer_high_cap(p);
	...
	if (target_cpu == -1 && most_spare_cap_cpu != -1 &&
		...
		!(p->state == TASK_RUNNING && !(available_idle_cpu(most_spare_cap_cpu)
				     ...
				     int sync, int sibling_count_hint)
{
	...
	int delta = 0;
	int task_boost = per_task_boost(p);
	int prefer_high_cap = schedtune_prefer_high_cap(p) || (task_boost > 0);
	int start_cpu;
...
unlock:
	...
	trace_sched_task_util(p, cpumask_bits(candidates)[0], best_energy_cpu,
			...
			placement_boost, start_t, prefer_high_cap, is_rtg,
